...
export const getSuggestedUser = async(req,res) =>{
    try {
        ...
        console.log('Error at suggesteduser:',error.message)
        res.status(500).json({error: error.message})
    }
}
export const updateUserProfile = async(req,res)=>{
    ...
    const userId = req.user._id;
    try {
        let user = await User.findById(userId); //error might happen in this line
        ...
        if(currentPassword && newPassword){
            const isMatch = await bcrypt.compare(currentPassword, user.password) //error might happen in this line
